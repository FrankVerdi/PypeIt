...
def test_apf_levy_final_config_frametypes():
    ...
    # Test case 2: decker is '3.0' with only wideflat frames
    # Should keep as 'pixelflat,trace'
    table2 = astropy.table.Table()
    table2['frametype'] = ['pixelflat,trace', 'science', 'arc']
    table2['filename'] = ['file1.fits', 'file2.fits', 'file3.fits']
    setup2 = {'decker': '3.0', 'binning': '1,1'}
    ...
    assert table2['frametype'][0] == 'pixelflat,trace', \
        "Wideflat frame should not be changed when only wideflat frames exist"

    # Test case 3: decker is '8.0'
    # Should NOT change wideflat frames even if narrowflat exists
    table3 = astropy.table.Table()
    table3['frametype'] = ['pixelflat,trace', 'trace', 'science']
    table3['filename'] = ['file1.fits', 'file2.fits', 'file3.fits']
    setup3 = {'decker': '8.0', 'binning': '1,1'}
    
    spec.final_config_frametypes(setup3, table3)
    
    # Check that wideflat frames were NOT changed
    assert table3['frametype'][0] == 'pixelflat,trace', \
        "Wideflat frame should NOT be changed when decker is not 3.0"
    assert table3['frametype'][1] == 'trace', \
        "Narrowflat frame should remain unchanged"
    
    # Test case 4: decker is '3.0' with multiple wideflat frames
    # All wideflat frames should be changed
    table4 = astropy.table.Table()
    table4['frametype'] = ['pixelflat,trace', 'pixelflat,trace', 'trace', 'science']
    table4['filename'] = ['file1.fits', 'file2.fits', 'file3.fits', 'file4.fits']
    setup4 = {'decker': '3.0', 'binning': '1,1'}
    
    spec.final_config_frametypes(setup4, table4)
    
    # Check that all wideflat frames were changed
    assert table4['frametype'][0] == 'pixelflat', \
        "First wideflat frame should be changed to pixelflat"
    assert table4['frametype'][1] == 'pixelflat', \
        "Second wideflat frame should be changed to pixelflat"
    assert table4['frametype'][2] == 'trace', \
        "Narrowflat frame should remain unchanged"
