...
allspec2d_det = fits.getval(spec2file, 'HIERARCH ALLSPEC2D_DETS')
detnames = allspec2d_det.split(',')

hdu_list = [fits.PrimaryHDU()]  # Empty primary HDU

for detname in detnames:
    print(f'Working on detector: {detname}')
    spec2d = spec2dobj.Spec2DObj.from_file(spec2file, detname, chk_version=False)
    ...
    if len(waves) == 0:
        print(f'There is a problem with the wavelengths on det {detname}. The ALLSLITS image will not be created.')
        continue
    ...
    # Create WCS header with wavelength information
    w = WCS(naxis=2)
    w.wcs.ctype = ['LINEAR', 'WAVE']
    w.wcs.crpix = [1, 1]
    w.wcs.crval = [0, wave_grid[0]]
    w.wcs.cdelt = [0, wave_grid[1] - wave_grid[0]]
    #w.wcs.cunit = ['pix', 'Angstrom']

    # Create HDU with WCS header
    hdu = fits.ImageHDU(image_rect, header=w.to_header(), name=detname)
    hdu.header['NSPEC'] = nspec_rect
    hdu.header['NSPAT'] = nspat_rect
    hdu.header['WAVEMIN'] = wave_grid[0]
    hdu.header['WAVEMAX'] = wave_grid[-1]
    hdu.header['DETECTOR'] = detname
